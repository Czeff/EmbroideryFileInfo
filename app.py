...
def timeout_handler(signum, frame):
    ...
    raise TimeoutError("Analysis timed out - plik może być zbyt duży lub złożony")
...
def try_pxf_analysis(file_path):
    ...
    try:
        ...
        analysis = {
            ...
            len(data),
            'format_detected':
            advanced_analysis['file_format'].get('description', 'PXF format'),
            'stitch_count':
            advanced_analysis['header_analysis'].get('stitch_count',
                                                     'Unknown'),
            'thread_count':
            advanced_analysis['header_analysis'].get('color_count', 0),
            'colors': [],
        ...
        if advanced_analysis['header_analysis']:
            ...
            analysis['detailed_info'] = {
                'software':
                advanced_analysis.get('file_format',
                                      {}).get('description', 'Unknown'),
                'format_version':
                advanced_analysis.get('file_format',
                                      {}).get('version', 'Unknown'),
                'header_size':
                header.get('header_size', 'Unknown'),
                'data_size':
                header.get('data_size', 'Unknown'),
                'estimated_stitches':
                header.get('stitch_count', 'Unknown'),
                'color_count':
                header.get('color_count', 0),
                'has_underlay':
                header.get('has_underlay', False),
                'has_applique':
                header.get('has_applique', False),
                'has_sequins':
                header.get('has_sequins', False)
            }
            ...
            if 'dimensions' in header:
                ...
                analysis['detailed_info'][
                    'hoop_size'] = f"{width_cm:.1f} × {height_cm:.1f} cm"
                analysis['dimensions'] = {
                    'width': width_cm,
                    'height': height_cm,
        ...
        if advanced_analysis['embroidery_parameters']:
            ...
            analysis['embroidery_parameters'] = {
                'row_spacing':
                params.get('stitch_density', 'Unknown'),
                'underlay_type':
                params.get('underlay_type', 'Unknown'),
                'pull_compensation':
                params.get('pull_compensation', 'Unknown'),
                'fill_angle':
                params.get('fill_angle', 'Unknown'),
                'auto_underlay':
                'Włączone' if analysis['detailed_info'].get('has_underlay')
                else 'Nieznane',
                'analysis_method':
                'Zaawansowana analiza PXF'
            }
        ...
        if advanced_analysis['stitch_data']:
            ...
            if 'dimensions' in stitch_data:
                ...
                height_cm = dims['height'] / 10.0
                analysis['detailed_info'][
                    'pattern_size'] = f"{width_cm:.1f} × {height_cm:.1f} cm"

            if 'average_stitch_length' in stitch_data:
                length_cm = stitch_data['average_stitch_length'] / 10.0
                analysis['detailed_info'][
                    'avg_stitch_length'] = f"{length_cm:.2f} cm"

            if 'coordinate_count' in stitch_data:
                analysis['detailed_info']['coordinates_found'] = stitch_data[
                    'coordinate_count']
        ...
        if advanced_analysis[
                'sections_found'] and 'colors' in advanced_analysis[
                    'sections_found']:
            ...
            try:
                ...
                if len(header) >= 32:
                    ...
                    try:
                        ...
                        val1 = struct.unpack('<I',
                                             header[8:12])[0]  # Position 8-11
                        val2 = struct.unpack(
                            '<I', header[12:16])[0]  # Position 12-15
                        val3 = struct.unpack(
                            '<I', header[16:20])[0]  # Position 16-19
                        val4 = struct.unpack(
                            '<I', header[20:24])[0]  # Position 20-23

                        # Filter reasonable dimension values (in 0.1mm units)
                        reasonable_dims = [
                            v for v in [val1, val2, val3, val4]
                            if 100 < v < 1000000
                        ]
                        if len(reasonable_dims) >= 2:
                            width_cm = reasonable_dims[
                                0] / 1000.0  # Konwersja na cm (z 0.1mm)
                            height_cm = reasonable_dims[1] / 1000.0
                            detailed_info[
                                'design_size'] = f"{width_cm:.1f} × {height_cm:.1f} cm"
                ...
                if created_pos != -1:
                    ...
                    try:
                        ...
                        if version_match:
                            detailed_info[
                                'software_version'] = f"DG{version_match.group(1)}"

                        header_info.append(
                            f'Software: {detailed_info.get("software", "Unknown")}'
                        )
                ...
                for i in range(0, len(data) - 4, 4):
                    try:
                        ...
                        if len(chunk) == 4:
                            ...
                            x = struct.unpack(
                                '<h', chunk[:2])[0] if len(chunk) >= 2 else 0
                            y = struct.unpack(
                                '<h', chunk[2:])[0] if len(chunk) >= 2 else 0
                ...
                if stitch_patterns > 0:
                    estimated_stitches = min(stitch_patterns // 4,
                                             50000)  # Conservative estimate
                    if estimated_stitches > 100:
                        analysis[
                            'stitch_count'] = f"~{estimated_stitches:,} (estimated)"
                        detailed_info[
                ...
                if stitch_patterns > 0 and 'design_size' in detailed_info:
                    try:
                        ...
                        area = float(dims[0].strip()) * float(dims[1].replace(
                            'cm', '').strip())
                        if area > 0:
                            density = estimated_stitches / area
                            detailed_info[
                                'stitch_density'] = f"{density:.1f} ściegów/cm²"
                    except:
                        pass
                ...
                if stitch_patterns > 0:
                    ...
                    if jump_ratio > 0.3:
                        detailed_info[
                            'fill_type'] = 'Złożony wzór z wypełnieniami i detalami'
                    elif jump_ratio > 0.1:
                ...
                while pos < len(data) - 6 and len(colors_found) < 20:
                    ...
                    if (pos + 2 < len(data) and data[pos] <= 255
                            and data[pos + 1] <= 255 and data[pos + 2] <= 255):
                ...
                if colors_found:
                    analysis[
                        'thread_count'] = f"{len(colors_found)} (detected)"
                    for i, color in enumerate(
                            colors_found[:10]):  # Limit to first 10 colors
                        analysis['colors'].append({
                            'index': i + 1,
                            'color': color['name'],
                            'hex': color['hex'],
                ...
                if 'estimated_stitches' in detailed_info:
                    ...
                    estimated_length = (detailed_info['estimated_stitches'] *
                                        0.3)  # in cm
                    if estimated_length > 100:
                        detailed_info[
                            'thread_consumption'] = f"{estimated_length/100:.1f} m"
                    else:
                        detailed_info[
                            'thread_consumption'] = f"{estimated_length:.0f} cm"
        ...
        analysis['layers'] = header_info if header_info else [
            'Basic file structure detected'
        ]
        analysis['detailed_info'] = detailed_info

        # Enhanced conversion note
        analysis[
            'conversion_note'] = 'Ten plik wymaga konwersji do formatu .dst, .pes lub .jef aby uzyskać pełne informacje o wzorze haftu.'
    ...
    try:
        ...
        if len(coordinates) > 10:
            ...
            if distances:
                ...
                results['parameters_found'][
                    'avg_stitch_length'] = f"{avg_distance/100:.2f} cm"
                results['parameters_found'][
                    'stitch_pattern_detected'] = f"{len(coordinates)} coordinate pairs"
...
def extract_pxf_embroidery_parameters(data):
    ...
    try:
        ...
        import re
        density_patterns = [
            r'density[:\s]*(\d+\.?\d*)', r'DENSITY[:\s]*(\d+\.?\d*)',
            r'stitch_density[:\s]*(\d+\.?\d*)',
            r'line_spacing[:\s]*(\d+\.?\d*)', r'spacing[:\s]*(\d+\.?\d*)'
        ]
        ...
        if not density_found:
            for i in range(0, len(data) - 16):
                    ...
                    try:
                        ...
                        if len(density_bytes) == 4:
                            ...
                            if 10 <= density_val <= 1000:  # Reasonable density range
                                density_cm = (density_val /
                                              100) / 10.0  # Konwersja na cm
                                params['row_spacing'] = f"{density_cm:.2f} cm"
                                density_found = True
                                break
                    except:
                ...
                if not density_found and (b'dens' in chunk.lower()
                                          or b'spac' in chunk.lower()):
                    try:
                        ...
                        for offset in range(-20, 21, 4):
                            if i + offset >= 0 and i + offset + 4 < len(data):
                                ...
                                if 0.1 <= float_val <= 20:  # Reasonable density range
                                    ...
                                    params[
                                        'row_spacing'] = f"{density_cm:.2f} cm"
                                    density_found = True
                                    break
                    except:
                        continue
    ...
    try:
        ...
        for i in range(0, len(data) - 32):
            ...
            if b'HOOP' in chunk or b'hoop' in chunk:
                ...
                hoop_sizes = [
                    '100x100', '130x180', '150x240', '200x300', '360x200'
                ]
                for size in hoop_sizes:
                    if size in hoop_info:
                        ...
                        h_cm = int(h) / 10.0
                        settings[
                            'hoop_dimensions'] = f"{w_cm:.1f}x{h_cm:.1f} cm"
                        break
...
def analyze_stitch_details(pattern):
    ...
    for stitch in pattern.stitches:
        if len(stitch) >= 3:
            ...
            if command == pyembroidery.STITCH:
                stats['stitch_commands'][
                    'Normal Stitch'] = stats['stitch_commands'].get(
                        'Normal Stitch', 0) + 1
            elif command == pyembroidery.JUMP:
                stats['jump_count'] += 1
                stats['stitch_commands'][
                    'Jump'] = stats['stitch_commands'].get('Jump', 0) + 1
            elif command == pyembroidery.COLOR_CHANGE:
                stats['color_changes'] += 1
                stats['stitch_commands'][
                    'Color Change'] = stats['stitch_commands'].get(
                        'Color Change', 0) + 1
            elif command == pyembroidery.TRIM:
                stats['trims'] += 1
                stats['stitch_commands'][
                    'Trim'] = stats['stitch_commands'].get('Trim', 0) + 1
            ...
            if prev_x is not None and prev_y is not None:
                ...
                if command == pyembroidery.JUMP:
                    jump_distances.append(distance)
                    stats['max_jump_distance'] = max(
                        stats['max_jump_distance'], distance)
    ...
    if stitch_distances:
        stats['avg_stitch_length'] = round(
            sum(stitch_distances) / len(stitch_distances) / 100,
            3)  # Convert to cm

    # Convert thread length to cm and then to more readable units
    stats['total_thread_length'] = round(stats['total_thread_length'] / 100,
                                         2)  # cm
    if stats['total_thread_length'] > 100:
        stats['total_thread_length_m'] = round(
            stats['total_thread_length'] / 100, 2)
...
def calculate_performance_metrics(pattern, dimensions):
    ...
    stitch_count = len(pattern.stitches)
    area = dimensions['width'] * dimensions[
        'height']  # Już w cm² jeśli dimensions są w cm

    # Calculate stitch density (stitches per cm²)
    if area > 0:
        metrics['stitch_density'] = round(stitch_count / area,
                                          1)  # ściegów na cm²
    ...
    color_changes = sum(
        1 for stitch in pattern.stitches
        if len(stitch) >= 3 and stitch[2] == pyembroidery.COLOR_CHANGE)
    trims = sum(1 for stitch in pattern.stitches
                if len(stitch) >= 3 and stitch[2] == pyembroidery.TRIM)
    ...
    if total_time_minutes < 60:
        metrics['estimated_time'][
            'total'] = f"{round(total_time_minutes)} minut"
    else:
    ...
    metrics['estimated_time'][
        'stitching'] = f"{round(base_time_minutes)} minut"
    metrics['estimated_time'][
        'setup'] = f"{round(additional_time * 60)} sekund"

    # Thread efficiency (lower jump-to-stitch ratio is better)
    jumps = sum(1 for stitch in pattern.stitches
                if len(stitch) >= 3 and stitch[2] == pyembroidery.JUMP)
    if stitch_count > 0:
        metrics['thread_efficiency'] = round((1 - jumps / stitch_count) * 100,
                                             1)

    # Color efficiency (fewer color changes relative to colors used is better)
    color_count = len(pattern.threadlist)
    if color_count > 0:
        metrics['color_efficiency'] = round(
            max(0, 100 - (color_changes / color_count) * 10), 1)
...
def analyze_embroidery_file(file_path):
    ...
    try:
        ...
        if pattern is None and file_ext == '.pxf':
            ...
            logging.info(
                "Attempting PXF to DST conversion for detailed analysis")
            ...
            if dst_file and os.path.exists(dst_file):
                ...
                if pattern is not None:
                    logging.info(
                        "Successfully converted and analyzed PXF file")
        ...
        analysis = {
            ...
            'layers': [],
            'dimensions': None,
            'converted_from_pxf': getattr(pattern, 'converted_from_pxf',
                                          False),
            'detailed_stats': {},
            'technical_info': {},
            'performance_metrics': {}
        ...
        for i, thread in enumerate(pattern.threadlist):
            color_info = {
                ...
                'hex':
                thread.hex if hasattr(thread, 'hex') and thread.hex else 'N/A',
                'description':
                thread.description if thread.description else 'N/A',
        ...
        if extends:
            ...
            analysis['dimensions'] = {
                'width': round(width / 10,
                               2),  # Convert to mm (assuming 0.1mm units)
        ...
        # Performance metrics
        performance = calculate_performance_metrics(pattern,
                                                    analysis['dimensions'])
        ...
        if not analysis['layers']:
            analysis['layers'] = [
                'Layer information not available in this file'
            ]
...
def upload_file():
    ...
    if not allowed_file(file.filename):
        flash(
            'Invalid file type. Please upload a supported embroidery file (.pxf, .dst, .pes, .jef, .exp, .vp3, .hus, .xxx).',
            'error')
        return redirect(url_for('index'))
